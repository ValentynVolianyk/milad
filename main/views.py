...
class CatalogView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update({
            ...
            'products': products,
            'current_categorie': category_slug,
            'filter_params': filter_params,
            'sizes': Size.objects.all(),
            'search_query': query or '',
        })

        if self.request.GET.get('show_serch') == 'true':
    ...
    def get(self, request, *args, **kwards):
        context = self.get_context_data(**kwards)
...
class ProductDetailViev(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context('categories') = Category.objects.all()
        context['related_peoducts'] = Product.objects.filter(
    ...
    def get(self, request, *args, **kwards):
        ...
        contex = self.get_context_data(**kwards)
        if request.header.get('HX-Request'):
            return TemplateResponse(request, 'main/product_detail.html', contex )
        raise TemplateResponse(request, self.template_name, contex)
